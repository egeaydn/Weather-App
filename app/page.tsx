...
export default function WeatherApp() {
  ...
  const fetchWeatherData = async (city: string) => {
    try {
      ...
      setError(`${city} için hava durumu verileri alınırken bir hata oluştu.`);
      setWeatherData(null);
    } finally {
      setLoading(false);
    }
              ...
              <div className="space-y-6 pb-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-gradient-to-br from-slate-800 to-gray-900 rounded-3xl p-6 text-white relative overflow-hidden border border-gray-700/30">
                      <div className="relative z-10">
                        <div className="flex items-center justify-between mb-4">
                          <div>
                            <h2 className="text-lg font-medium opacity-90">Şu an</h2>
                  ...
                  <div className="lg:col-span-2">
                    <div className="mb-4">
                      <h3 className="text-white text-lg font-semibold mb-4">Bugünün Özeti</h3>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {/* Wind Status */}
                        <div className="bg-black/60 backdrop-blur-xl rounded-2xl p-4 border border-gray-800/50">
                          <div className="flex justify-between items-start mb-2">
                            <p className="text-gray-400 text-sm">Rüzgar Durumu</p>
                          </div>
                        ...
                        <div className="bg-black/60 backdrop-blur-xl rounded-2xl p-4 border border-gray-800/50">
                          <div className="flex justify-between items-start mb-2">
                            <p className="text-gray-400 text-sm">UV İndeksi</p>
                          </div>
                        ...
                        <div className="bg-black/60 backdrop-blur-xl rounded-2xl p-4 border border-gray-800/50">
                          <div className="flex justify-between items-start mb-2">
                            <p className="text-gray-400 text-sm">Gün Doğumu & Batımı</p>
                          </div>
                          <div className="space-y-2">
                            <div className="flex items-center space-x-2">
                              <div className="w-3 h-3 bg-yellow-400 rounded-full"></div>
                              <span className="text-white text-sm">
                                Doğum: {weatherData.sys?.sunrise ? formatTime(weatherData.sys.sunrise) : '06:30'}
                              </span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <div className="w-3 h-3 bg-orange-400 rounded-full"></div>
                              <span className="text-white text-sm">
                                Batım: {weatherData.sys?.sunset ? formatTime(weatherData.sys.sunset) : '19:30'}
                              </span>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {/* Humidity */}
                      <div className="bg-black/60 backdrop-blur-xl rounded-2xl p-4 border border-gray-800/50">
                        <p className="text-gray-400 text-sm mb-2">Nem</p>
                        <p className="text-white text-2xl font-bold">
                          {weatherData.main?.humidity || 0}
                          <span className="text-sm font-normal">%</span>
                        </p>
                        <p className="text-gray-400 text-xs mt-2">Normal seviye</p>
                      </div>

                      {/* Visibility */}
                      <div className="bg-black/60 backdrop-blur-xl rounded-2xl p-4 border border-gray-800/50">
                        <p className="text-gray-400 text-sm mb-2">Görüş Mesafesi</p>
                        <p className="text-white text-2xl font-bold">
                          {getVisibility()}
                          <span className="text-sm font-normal ml-1">km</span>
                        </p>
                        <p className="text-gray-400 text-xs mt-2">İyi görüş</p>
                      </div>

                      {/* Feels Like */}
                      <div className="bg-black/60 backdrop-blur-xl rounded-2xl p-4 border border-gray-800/50">
                        <p className="text-gray-400 text-sm mb-2">Hissedilen</p>
                        <p className="text-white text-2xl font-bold">
                          {Math.round(weatherData.main?.feels_like || 0)}°
                          <span className="text-sm font-normal">C</span>
                        </p>
                        <p className="text-gray-400 text-xs mt-2">Gerçek gibi</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {forecastData && (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-black/60 backdrop-blur-xl rounded-2xl p-6 border border-gray-800/50">
                      <h3 className="text-white text-lg font-semibold mb-4">7 Günlük Tahmin</h3>
                            ...
                            <div className="flex items-center space-x-3">
                              ...
                              <div>
                                <p className="text-white font-medium">
                                  {index === 0 ? 'Bugün' : getDayName(item.dt_txt)}
                                </p>
                    ...
                    <div className="bg-black/60 backdrop-blur-xl rounded-2xl p-6 border border-gray-800/50">
                      <div className="flex justify-between items-center mb-4">
                        <h3 className="text-white text-lg font-semibold">Hava Durumu Haritası</h3>
                      ...
                      <div className="h-48 bg-gradient-to-br from-blue-900/50 to-indigo-900/50 rounded-xl relative overflow-hidden">
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="text-center">
                            ...
                            <p className="text-gray-400 text-sm">%{weatherData?.main?.humidity || 0} nem</p>
                          </div>
                        </div>
                      </div>
                    </div>
